...
Lights::Lights(int powerPin, int modePin, int backLightPin, LedStrip &frontLightStrip, LedStrip &dashboardStrip, LedStrip &plateStrip)
    : m_headlightPin(powerPin),
      m_backLightPin(backLightPin),
      m_highBeamPin(modePin),
...
void Lights::set(LightsMode mode)
{
    ...
    case LIGHTS_CITY:
        ...
        digitalWrite(m_highBeamPin, LOW);                                  // High beam OFF
        m_frontLightStrip.setColor(COLOR_OFF);                             // Front LED strip off, headlight is on
        m_dashboardStrip.setColor(COLOR_WHITE, CITY_DASHBOARD_BRIGHTNESS); // Dashboard LED strip adapted to city mode
        m_plateStrip.setColor(COLOR_WHITE, CITY_PLATE_BRIGHTNESS);         // Plate LED strip adapted to city mode
        break;
    case LIGHTS_COUNTRY:
        digitalWrite(m_headlightPin, HIGH);                                // Headlight ON
        digitalWrite(m_backLightPin, HIGH);                                // Back light ON
